...
IMG_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "chapters", "img")
...
MARKETS = [
    "Hong Kong", "Mainland China", "Taiwan", "Singapore", "Malaysia", "Australia",
    "India", "Indonesia", "Japan", "South Korea", "Thailand", "Vietnam", "Philippines", "New Zealand",
]
...
ax.barh(y, RETAIL,                     color=HSBC_RED,   label="Retail Banking")
ax.barh(y, CORPORATE, left=RETAIL,     color=HSBC_BLUE,  label="Corporate Banking")
left2 = [r + c for r, c in zip(RETAIL, CORPORATE)]
ax.barh(y, WEALTH,    left=left2,      color="#FFB81C",  label="Wealth Management")
left3 = [l + w for l, w in zip(left2, WEALTH)]
ax.barh(y, ISLAMIC,   left=left3,      color=HSBC_GREEN, label="Islamic Finance")
...
ax.set_xlabel("Business Mix (%)", fontsize=11, color=HSBC_GREY)
ax.set_title("Comparison of Business Composition Across HSBC’s Key AME Markets", fontsize=15,
...
ax.text(REL_SIZE[0] + 2, 61, "Current Avg ~60%", fontsize=8, color=HSBC_RED, va="bottom")
ax.axhline(y=90, color=HSBC_GREEN, linestyle="--", alpha=0.4, linewidth=1.2)
ax.text(REL_SIZE[0] + 2, 91, "Target 90%+", fontsize=8, color=HSBC_GREEN, va="bottom")

ax.set_xlabel("Relative Business Scale (Hong Kong = 100)", fontsize=11, color=HSBC_GREY)
ax.set_ylabel("Regulatory Reporting Automation Rate (%)", fontsize=11, color=HSBC_GREY)
ax.set_title("HSBC APAC: Business Size vs Automation Rate", fontsize=15,
             fontweight="bold", color=HSBC_RED, pad=16)

# Custom legend
legend_elements = [
    Line2D([0], [0], marker='o', color='w', markerfacecolor=HSBC_RED,  markersize=14, label='Flagship Market (Hong Kong)'),
    Line2D([0], [0], marker='o', color='w', markerfacecolor=HSBC_BLUE, markersize=12, label='Key Markets'),
    Line2D([0], [0], marker='o', color='w', markerfacecolor=HSBC_GREY, markersize=9,  label='Other Markets'),
    Line2D([0], [0], color=HSBC_RED,  linestyle='--', linewidth=1.2, label='Current Average Automation Rate (~60%)'),
    Line2D([0], [0], color=HSBC_GREEN, linestyle='--', linewidth=1.2, label='Target Automation Rate (90%+)'),
]
...
bars1 = ax.bar(x - width/2, REG_INTENSITY,  width,
               color=HSBC_RED, alpha=0.85, label="Regulatory Reporting Intensity Index")
bars2 = ax.bar(x + width/2, BIZ_COMPLEXITY, width,
               color=HSBC_BLUE, alpha=0.85, label="Business Complexity Index")
...
ax.set_ylabel("Index (0-100)", fontsize=11, color=HSBC_GREY)
ax.set_title("HSBC APAC: Regulatory Intensity vs Business Complexity vs Data Localisation", fontsize=15,
             fontweight="bold", color=HSBC_RED, pad=16)
